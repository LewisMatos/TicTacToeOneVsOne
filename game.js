...
//When the user clicks a box it'll display a piece on the board depending on which user.
    function clickBox(id){
        //if winner is not true then check the board for the id that he user clicked and check to see who's turn it is.
        //Adds the picture of the board either x or o depending on player and then checks to see if that player won the match.  
        if(!winner){
            for(var i = 1; i < board.length; i++){
                for(j = 1; j < board.length; j++){
                    if(id == board[i][j]){
                        ...
                        }if(turn == "O" && document.getElementById(board[i][j]).innerHTML == ""){
                            document.getElementById(board[i][j]).innerHTML = "<img src =" + O.src + ">";
